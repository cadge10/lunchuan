...
if (base::p('action')=='add') {
	...
	$sql = "update ###_producttype set title='$title',sortid='$sortid',url='$url',isshow='$isshow' where id='$id'";
	$db->query($sql);
	
	exit(base::alert('修改成功，请返回！','?mod=admin_product_type'));
}

$id = (int)base::g('id');
if ($id==0) exit(base::alert('参数传递错误！'));
$one = $db->get_one("select * from ###_producttype where id='$id'");
...
<table cellspacing=0 cellpadding=0 width="100%" border=0 class="listtable">
  <tbody> 
    ...
    <tr>
      ...
      <td><?php echo product::getartcol_p($one['typeid'],'默认一级栏目');?> *栏目不能修改。<script type="text/javascript">document.form1.typeid.disabled=true;</script></td>
    </tr>
    <tr>
