...
<?php
// 执行语句
$formatdata = product::getnewstypearray_p();
?>
<html>
<head>
...
?>
      <tr class="darkrow"> 
        ...
        <td align="center"><a href="?mod=admin_product&typeid=<?php echo $tree['id'];?>"><u><?php echo product::gettypenewsdall_p($tree['id'])?></u></a></td>
        <td><?php echo $tree['url']?></td>
        <td align="center"><?php echo $tree['isshow']==1?'<span style="color:blue;">显示</span>':'<span style="color:gray;">不显示</span>'?></td>
        <td align="center"><a href="?mod=admin_product_type_add&parentid=<?php echo $tree['id']?>"><u>添加子栏目</u></a> | <a href="?mod=admin_product_type_edit&id=<?php echo $tree['id']?>"><u>编辑</u></a> | <a href="?mod=delete&table=producttype&action=admin_product_type_del&delid=<?php echo $tree['id']?>" onClick="return confirm('您确定要删除吗？该操作同时删除此栏目下的所有文章')"><u>删除</u></a></td>
      </tr>
<?php } } else {?>
      <tr>
