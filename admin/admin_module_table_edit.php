...
if (base::p("action")) {
	...
	unset($post_data['id'],$post_data['action'],$post_data['Submit'],$post_data['style_color'],$post_data['style_bold']);
	...
	$sql = "UPDATE ###_{$table} SET ".implode(',',$new_data_u)." WHERE id='$id'";
	
	
	$gourl = trim(base::p('gourl'));
	
	if ($db->query($sql)) {
		exit(base::alert('数据修改成功，请返回。','?mod=admin_module_table&moduleid='.$moduleid.$gourl));
	} else {
...
$id = (int)base::g('id');
if ($id==0) exit(base::alert('参数传递错误！'));
$one = $db->get_one("select * from ###_{$table} where id='$id'");
