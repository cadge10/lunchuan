...
class template
{
    ...
    var $_var           = array();
    var $_hash        = '18cbbee3b2d14f83679875b11869e261';
    var $_foreach       = array();
    var $_current_file  = '';
    var $_expires       = 0;
    var $_errorlevel    = 0;
    ...
    function fetch($filename, $cache_id = '')
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
					if ($this->_nowtime >= $this->_expires) {
						$this->_current_file = $filename;
						$out = $this->_eval($this->fetch_str(file_get_contents($filename)));
					} else {
    ...
    function select($tag)
    {
        ...
        elseif ($tag{0} == '/') // 结束 tag
        {
            switch (substr($tag, 1))
            {
				...
				case function_exists(substr($tag, 1)) :
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                case function_exists($tag_sel) :
                    ...
                    return $this->_compile_foreach_start($tag);
					break;
                ...
                case 'include':
					...
					// THEME_NAME自动替换模板主题名称
					$template_dir = isset($GLOBALS['template_dir'])?$GLOBALS['template_dir']:'default';
					$t_file = str_replace('THEME_NAME',$template_dir,$t[file]);
                    if(@substr($t[file],-4,4)!='html')
                    {
	                    return '<?php echo $this->fetch(' . "$t_file" . '); ?>';
	                    break;
                    }
                    else
                    {
                ...
                case 'insert' :
                    ...
                    $out = "<?php \n" . '$k = ' . preg_replace("/(\'\\$[^,]+)/e" , "stripslashes(trim('\\1','\''));", var_export($this->_parse_vars_r($t), true)) . ";\n";
        ...
        if (!empty($moddb))
        {
            foreach ($moddb as $key => $mod)
            {
                ...
                switch ($s[0])
                {
                    ...
                    case 'truncate':
                        $p = 'base::sub_str(' . $p . ",$s[1])";
                        break;
						
					case 'date_format':
						$s[1]=$s[1]?str_replace('：',':',$s[1]):"Y-m-d";
                        $p = 'date("'.$s[1].'",'.$p.')';
                        break;
    ...
    function _compile_foreach_start($tag_args)
    {
        ...
        if (array_key_exists('lib',$attrs)) { // 使用 function 参数
            $_attrs = $attrs;
	        unset($_attrs['lib']);
	        unset($_attrs['from']);
	        unset($_attrs['item']);
	        ...
	        if (!empty($_attrs)) {
	        	...
	        	foreach ($_attrs as $k=>$v) {
	        		...
	        		$_at .= '"'.$k.'" => '.$this->_parse_vars_r($_v).',';
	        	}
	        	$_arr .= trim($_at,',');
	        }
	        $_arr .= ")";
        	$func_data = $attrs['lib']."(".$_arr.")";
        	$from = $func_data;
        } else {
        	$from = $attrs['from'];
        }
    ...
    function str_trim($str)
    {
        ...
        if ($matches) {
        	foreach ($matches[0] as $v) {
        		$_v = str_replace(" ", "　", $this->_parse_vars($v));
        		$str = str_replace($v, $_v, $str);
        	}
        }
    ...
    function _parse_vars($str) {
    	$where = $str;
		$where = str_ireplace('>=', '__gte__', $where);
		$where = str_ireplace('>=', '__ge__', $where);
		$where = str_ireplace('<=', '__lte__', $where);
		$where = str_ireplace('<=', '__le__', $where);
		$where = str_ireplace('=', '__eq__', $where);
		$where = str_ireplace("'", '$$$$$', $where);
		return $where;
    }
    
	function _parse_vars_r($str) {
    	$where = $str;
		$where = str_ireplace('__eq__', '=', $where);
		$where = str_ireplace('__gte__', '>=', $where);
		$where = str_ireplace('__ge__', '>=', $where);
