...
Class Price{
    ...
    /**
     * 报价列表
     * @url http://api请求地址/api.php?mod=Price&fun=getPriceList&page=xxx&size=xxx&time=xxxx&token=xxxx
     * @example price1.html How to use this function
     * @param int $page 页
     * @param int $size 每页数
     * @return array{code->状态码,msg->提示信息,data->数据数组}
     */
	public function getPriceList($page=0,$size=0){
		...
		$sql	=	"SELECT id,price FROM ###_".$this->table." WHERE 1 ORDER BY id DESC $limit";
     ...
     * @url http://api请求地址/api.php?mod=Users&fun=getPriceInfo&id=xxx&time=xxxx&token=xxxx
     * @example price2.html How to use this function
     * @param Int $id 用户ID
     * @return array{code->状态码,msg->提示信息,data->数据数组}
     */
	public function getPriceInfo($id=0){
