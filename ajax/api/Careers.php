...
Class Careers {
    ...
    /**
     * 人才列表
     * @url http://api请求地址/api.php?mod=Careers&fun=getCareersList&page=xxx&size=xxx&time=xxxx&token=xxxx
     * @example price1.html How to use this function
     * @param int $page 页
     * @param int $size 每页数
     * @return array{code->状态码,msg->提示信息,data->数据数组}
     */
	public function getCareersList($page=0,$size=0){
		...
		$sql	=	"SELECT id,title FROM ###_".$this->table." WHERE 1 ORDER BY id DESC $limit";
     ...
     * @url http://api请求地址/api.php?mod=Careers&fun=getCareersInfo&id=xxx&time=xxxx&token=xxxx
     * @example products2.html How to use this function
     * @param Int $id 用户ID
     * @return array{code->状态码,msg->提示信息,data->数据数组}
     */
	public function getCareersInfo($id=0){
