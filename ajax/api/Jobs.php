...
Class Jobs{
    ...
    /**
     * 招聘列表
     * @url http://api请求地址/api.php?mod=Jobs&fun=getJobsList&page=xxx&size=xxx&time=xxxx&token=xxxx
     * @example price1.html How to use this function
     * @param int $page 页
     * @param int $size 每页数
     * @return array{code->状态码,msg->提示信息,data->数据数组}
     */
	public function getJobsList($page=0,$size=0){
     ...
     * @url http://api请求地址/api.php?mod=Jobs&fun=getJobsInfo&id=xxx&time=xxxx&token=xxxx
     * @example products2.html How to use this function
     * @param Int $id 用户ID
     * @return array{code->状态码,msg->提示信息,data->数据数组}
     */
	public function getJobsInfo($id=0){
